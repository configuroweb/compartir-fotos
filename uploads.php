...
if (!isset($_SESSION['login_firstname']) || $_SESSION['login_firstname'] != 'Mauricio') {
	die("<h1>Acceso denegado. Tu perfil solo admite que puedas ver las fotografías compartidas</h1>	");
}
		...
		<div class="container">
			<div class="col-lg-12">
					...
					<div class="col-md-8 offset-md-2">
						<div class="form-group">
							<select name="type" id="" class="badge badge-secondary">
								<option value="1">Comparte tu foto</option>
								<option value="0">Sube tu foto sin compartir</option>
							</select>
						</div>
					</div>
				</div>
				<div class="row">
					<div class="col-md-8 offset-md-2">
						<div class="form-group">
							<textarea name="content" id="" cols="30" rows="4" placeholder="Escribe un comentario para tu foto aquí" class="form-control"></textarea>
						</div>
					</div>
				</div>
				...
				<div id="drop">
					<span id="dname" class="text-center">Deja tus archivos aquí <br> or <br> <label for="chooseFile"><strong>Examinar</strong></label></span>
				</div>
				<div id="list"></div>
				<div class="col-md-12">
					<button class="btn btn-sm btn-primary btn-block col-md-2  offset-md-10"><i class="fa fa-upload"></i> Subir archivo</button>
				</div>
...
<script>
	...
	$('#frm-upload').submit(function(e) {
		...
		$.ajax({
			...
			success: function(resp) {
				if (resp == 1) {
					alert_toast("Archivo subido correctamente", 'success')
					setTimeout(function() {
						location.reload()
					}, 1500)
	...
	if (window.FileReader) {
		...
		document.getElementById('status').innerHTML = 'Tu navegador no es compatible con HTML5 FileReader.';
	}
	...
	function rem_func(_this) {
		...
		if ($('#drop .imgF').length <= 0) {
			$('#drop').append('<span id="dname" class="text-center">Deja tus archivos aquí <br> or <br> <label for="chooseFile"><strong>Examinar</strong></label></span>')
		}
	}
	$(document).ready(function() {
		$('.btn-share').click(function() {
			uni_modal("Escribe el contenido de tu publicación", 'share.php?id=' + $(this).attr('data-id'))
		})
	})
</script>